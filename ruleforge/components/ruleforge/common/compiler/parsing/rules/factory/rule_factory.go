...
func sliceContains[T shared.TokenTypeConstraint](s []T, e T) bool {
	for _, a := range s {
		if a == e {
			return true
		}
...
type ParsingRuleFactory[T shared.TokenTypeConstraint] struct {
}

func NewParsingRuleFactory[T shared.TokenTypeConstraint]() *ParsingRuleFactory[T] {
...
func (p *ParsingRuleFactory[T]) NewNestedParsingRule(
	symbol string,
	childRules []rules.ParsingRuleInterface[T]) rules.ParsingRuleInterface[T] {
